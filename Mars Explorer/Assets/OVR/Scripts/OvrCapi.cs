...
namespace Ovr
{
    ...
    public class Hmd : IDisposable
    {
		...
		{
			return ovrHmd_AttachToWindow(HmdPtr, default(IntPtr), destMirrorRect, sourceRenderTargetRect) != 0;
		}
		...
		{
			return ovrHmd_GetTrackingState(HmdPtr, 0.0d);
		}
		...
		public Sizei GetFovTextureSize(Eye eye, FovPort fov)
		{
			return ovrHmd_GetFovTextureSize(HmdPtr, eye, fov, 1.0f);
		}
		...
		{
			FrameTiming_Raw raw = ovrHmd_BeginFrame(HmdPtr, 0);
			return new FrameTiming(raw);
		}
		...
		{
			return ovrHmd_GetBool(HmdPtr, propertyName, Convert.ToSByte(false)) != 0;
		}
		...
		{
			return ovrHmd_GetInt(HmdPtr, propertyName, 0);
		}
		...
		{
			return ovrHmd_GetFloat(HmdPtr, propertyName, 0.0f);
		}
